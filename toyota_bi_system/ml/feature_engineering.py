...
class FeatureEngineer:
    ...
    def _print_quality_report(self, df, target_col='SalePrice'):
        """Menampilkan laporan kualitas data dengan format berbeda."""
        print("\n" + "#"*70)
        print(">>> QA AUDIT LOGS: DATA HEALTH CHECK")
        print("#"*70)

        # 1. INTEGRITY CHECK (MISSING VALUES)
        print(f"\n[*] CHECK 1: DATA INTEGRITY (NULL VALUES)")
        missing_count = df.isnull().any(axis=1).sum()
        status_icon = "[OK]" if missing_count == 0 else "[!!]"
        print(f"    {status_icon} Total Rows with Nulls : {missing_count}")
        
        # 2. ANOMALY DETECTION (OUTLIERS)
        print(f"\n[*] CHECK 2: ANOMALY DETECTION (IQR METHOD)")
        ...
        if target_col in df.columns:
            ...
            outliers = df[(df[target_col] < low_limit) | (df[target_col] > high_limit)]
            cnt_out = len(outliers)
            
            print(f"    > Thresholds      : Low [{low_limit:,.0f}] | High [{high_limit:,.0f}]")
            print(f"    > Flagged Records : {cnt_out} ({cnt_out/len(df):.2%})")
            
            if cnt_out > 0:
                print("    > Top Deviations  :")
                outliers = outliers.copy()
                outliers['dev'] = (outliers[target_col] - df[target_col].mean()).abs()
                
                for i, (idx, row) in enumerate(outliers.sort_values('dev', ascending=False).head(3).iterrows(), 1):
                    model_info = f"{row.get('CarModel', 'Unk')} '{str(row.get('CarYear', ''))[-2:]}"
                    print(f"      {i}. ID:{idx:<4} | {model_info:<15} | Val: {row[target_col]:,.0f}")
                
                print("    > Action          : RETAIN (Algorithm Robustness)")
        else:
            print("    [!!] Target column not found.")

        # 3. STABILITY CHECK (DATA DRIFT)
        print(f"\n[*] CHECK 3: DISTRIBUTION STABILITY (KS-TEST)")
        
        mid = len(df) // 2
        d_ref = df[target_col].iloc[:mid]
        d_cur = df[target_col].iloc[mid:]
        
        ks_val, p_val = stats.ks_2samp(d_ref, d_cur)
        
        drift_status = "STABLE" if p_val > 0.05 else "DRIFT DETECTED"
        drift_icon = "[OK]" if p_val > 0.05 else "[!!]"
        
        print(f"    > P-Value         : {p_val:.4f} (Alpha: 0.05)")
        print(f"    > Conclusion      : {drift_icon} {drift_status}")
...
def main():
    ...
    engineer._print_quality_report(df_full, target_col='SalePrice')
